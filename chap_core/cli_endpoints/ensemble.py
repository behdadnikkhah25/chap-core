...
def evaluate_ensemble(
    ...
    if model_configuration_yaml is not None:
        ...
        configuration = ModelConfiguration.model_validate(
            yaml.safe_load(open(model_configuration_yaml))
        )
    ...
    # save_results forventer: {model_name: (aggregat_metrics, detaljert_df)}
    results_dict = {"ensemble": (metrics_dict, flat.forecasts)}
    save_results(str(report_filename), results_dict)

    logger.info(f"Ensemble evaluation complete. Results saved to {report_filename}")
    return results_dict
